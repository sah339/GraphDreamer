...
class SaverMixin:
    ...
    def save_image_grid(
        ...
        cv2.imwrite(filepath, img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
        
        # img = (img + 1.0) / 2.0
        # image = Image.fromarray((img * 255).astype(numpy.uint8))
    ...
    def save_image(self, filename, img, no_compress=False):
        img = self.convert_data(img)
        ...
        if img.ndim == 3 and img.shape[-1] == 3:
            ...
            img = cv2.cvtColor(img, cv2.COLOR_RGBA2BGRA)
        if no_compress:
            cv2.imwrite(self.get_save_path(filename), img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
        else:
            cv2.imwrite(self.get_save_path(filename), img)
