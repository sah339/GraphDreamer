...
class ExperimentConfig:
    ...
    def __post_init__(self):
        ...
        self.trial_name = self.timestamp + self.trial_name[:35]
        self.exp_dir = os.path.join(self.exp_root_dir, self.name)
        self.trial_dir = os.path.join(self.exp_dir, self.trial_name)

        # os.environ["RM"] = self.trial_dir
        # os.putenv('RM',self.trial_dir)
        # os.system('bash')
        
        cmd = f"export RM={self.trial_dir}; echo $RM"; os.system(cmd)
