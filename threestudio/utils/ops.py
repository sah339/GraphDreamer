...
def get_activation(name) -> Callable:
    ...
    elif name == "softmax": #FIXME
        return lambda x: F.softmax(x - x.max(dim=-1, keepdim=True).values, dim=-1)
    else:
        try:
...
def chunk_batch(func: Callable, chunk_size: int, *args, **kwargs) -> Any:
    ...
    for i in range(0, max(1, B), chunk_size):
        ...
        for k, v in out_chunk.items():
            ...
            #     v = v if torch.is_grad_enabled() else [vv.detach() for vv in v]
            # else:
...
def aligned_component(x: Float[Tensor, "B C H W"], y: Float[Tensor, "B C H W"]):  # TODO
    # get the component of x that is aligned with y
    eps = torch.ones_like(x[:, 0, 0, 0]) * 1e-6
    return (
        (
            torch.mul(x, y).sum(dim=[1, 2, 3])
