...
class StableDiffusionGuidance(BaseObject):
    ...
    def compute_grad_sds(
        ...
        if prompt_utils.use_perp_neg:
            ...
            noise_pred = noise_pred_uncond + guidance_w * (
                e_pos + accum_grad
            )
        else:
            ...
            noise_pred = noise_pred_text + guidance_w * (
        ...
        **kwargs,
        ...
        if self.cfg.use_sjc:
            ...
            grad, guidance_eval_utils = self.compute_grad_sds(
                latents, t, prompt_utils, elevation, azimuth, camera_distances, scale_up
            )
    ...
    def get_noise_pred(
        ...
        if use_perp_neg:
            ...
            noise_pred = noise_pred_uncond + self.guidance_w * (
                e_pos + accum_grad
            )
        else:
            # pred noise
            ...
            # perform guidance (high scale from paper!)
            noise_pred_text, noise_pred_uncond = noise_pred.chunk(2)
            noise_pred = noise_pred_text + self.guidance_w * (
                noise_pred_text - noise_pred_uncond
            )
