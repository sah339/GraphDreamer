...
class DeepFloydGuidance(BaseObject):
    ...
    def _forward_multiple_prompts(
        self,
        rgb: Float[Tensor, "B H W C"],
        prompt_utils_list: Optional[PromptProcessorOutput],
        # **batch:
        elevation: Float[Tensor, "B"],
        azimuth: Float[Tensor, "B"],
        camera_distances: Float[Tensor, "B"],
        ...
        **kwargs,
        ...
        # guidance_w = guidance_scale if guidance_scale > 0. else self.cfg.guidance_scale
        guidance_w = guidance_scale if guidance_scale > 0. else self.guidance_w
        ...
        K = len(prompt_utils_list)
        noise_pred_text_all = torch.zeros_like(latents).to(latents)
        noise_pred_uncond_all = torch.zeros_like(latents).to(latents)
        e_pos_all = torch.zeros_like(latents).to(latents)
        accum_grad_all = torch.zeros_like(latents).to(latents)
        
        for prompt_utils in prompt_utils_list:
            accum_grad = 0
            ...
            if prompt_utils.use_perp_neg:
                ...
                # noise_pred = noise_pred_uncond + guidance_w * (e_pos + accum_grad)
            else:
                text_embeddings = prompt_utils.get_text_embeddings(
                    elevation, azimuth, camera_distances, self.cfg.view_dependent_prompting
                )  # 2B, N, Nf
            ...
            noise_pred_text_all += noise_pred_text
            noise_pred_uncond_all += noise_pred_uncond
            accum_grad_all += accum_grad
            e_pos_all += e_pos    

        ''''''
        if prompt_utils.use_perp_neg:
            noise_pred = noise_pred_uncond_all + guidance_w * (e_pos_all + accum_grad_all)
        else:
            noise_pred = noise_pred_text_all + guidance_w * e_pos_all
            # = noise_pred_text + guidance_w * (noise_pred_text - noise_pred_uncond)
            # = (1 + guidance_w) * noise_pred_text - guidance_w * noise_pred_uncond
    ...
    def forward_multiple_prompts(
        ...
        **kwargs,
        ...
        grad_all = torch.zeros_like(latents).to(latents)
        for prompt_utils in prompt_utils_list:
            # add noise
            noise = torch.randn_like(latents)
            latents_noisy = self.scheduler.add_noise(latents, noise, t)
    ...
    def __forward_multiple_prompts(
        self,
        rgb: Float[Tensor, "B H W C"],
        prompt_utils_list: Optional[PromptProcessorOutput],
        # **batch:
        ...
        **kwargs,
        ...
        for prompt_utils in prompt_utils_list:
            ...
            if prompt_utils.use_perp_neg:
                (
                    text_embeddings,
                    neg_guidance_weights,  # batch_size, 2
                ) = prompt_utils.get_text_embeddings_perp_neg(
                    elevation, azimuth, camera_distances, self.cfg.view_dependent_prompting)
                
                _embeddings = text_embeddings[:batch_size]
                _embeddings_uncond = text_embeddings[batch_size : batch_size * 2]
                _embeddings_neg = text_embeddings[batch_size * 2 :]      

                neg_weights = neg_guidance_weights if neg_weights is None else torch.cat([neg_weights, neg_guidance_weights], dim=0) 
                ...
                _embeddings = text_embeddings[:batch_size]
                _embeddings_uncond = text_embeddings[batch_size:]
                _embeddings_neg = None
            
            embeddings = torch.cat([embeddings, _embeddings], dim=0) if embeddings is not None else _embeddings
            embeddings_uncond = torch.cat([embeddings_uncond, _embeddings_uncond], dim=0) if embeddings_uncond is not None else _embeddings_uncond
            embeddings_neg = torch.cat([embeddings_neg, _embeddings_neg], dim=0) if embeddings_neg is not None else _embeddings_neg

        if prompt_utils.use_perp_neg:
                
            with torch.no_grad():
                latent_model_input = torch.cat([latents_noisy] * 4 * K, dim=0)
                noise_pred = self.forward_unet(
                    latent_model_input,
                    torch.cat([t] * 4 * K),
                    encoder_hidden_states=torch.cat([embeddings, embeddings_uncond, embeddings_neg], dim=0),
                )  # (4B*K, 6, 64, 64)

            # FIXME:
            noise_pred_text = noise_pred[:batch_size*K].view(K, batch_size, *noise_pred.shape[1:]).sum(0)
            noise_pred_uncond = noise_pred[batch_size*K : batch_size*2*K].view(K, batch_size, *noise_pred.shape[1:]).sum(0)
            noise_pred_neg = noise_pred[batch_size*2*K :].view(K, -1, *noise_pred.shape[1:]).sum(0) 

            noise_pred_text, _ = noise_pred_text.split(3, dim=1)
            noise_pred_uncond, _ = noise_pred_uncond.split(3, dim=1)
            noise_pred_neg, _ = noise_pred_neg.split(3, dim=1)

            neg_weights = neg_weights.view(K, batch_size, -1).sum(0)  # B, 2

            e_pos = noise_pred_text - noise_pred_uncond
            accum_grad = 0
            
            n_negative_prompts = neg_weights.shape[-1]

            for i in range(n_negative_prompts):
                e_i_neg = noise_pred_neg[i::n_negative_prompts] - noise_pred_uncond 
                accum_grad += neg_weights[:, i].view(
                    -1, 1, 1, 1
                ) * perpendicular_component(e_i_neg, e_pos)  # get the component of x that is perpendicular to y  
            ...
            with torch.no_grad():
                # pred noise
                latent_model_input = torch.cat([latents_noisy] * 2 * K, dim=0)
                noise_pred = self.forward_unet(
                    latent_model_input,
                    torch.cat([t] * 2 * K),
                    encoder_hidden_states=torch.cat([embeddings, embeddings_uncond], dim=0),
                )  # (KB+KB, 6, 64, 64)

            # perform guidance (high scale from paper!)
            noise_pred_text, noise_pred_uncond = noise_pred.chunk(2)  # KB, 6, 64, 64

            noise_pred_text = noise_pred_text.view(K, -1, *noise_pred_text.shape[1:]).sum(0)  # B, 6, 64, 64
            noise_pred_uncond = noise_pred_uncond.view(K, -1, *noise_pred_uncond.shape[1:]).sum(0)  # B, 6, 64, 64
            
            noise_pred_text, _ = noise_pred_text.split(3, dim=1)  # B, 3, 64, 64
            noise_pred_uncond, _ = noise_pred_uncond.split(3, dim=1)
