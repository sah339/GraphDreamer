...
class PromptProcessorOutput:
    ...
    def get_text_embeddings(
        ...
        return torch.cat([text_embeddings, uncond_text_embeddings], dim=0) # 2B, N, Nf
...
class PromptProcessor(BaseObject):
    ...
    @rank_zero_only
    def prepare_text_embeddings(self, pooled=False):  # tokenize and save embeddings
        ...
        for prompt in all_prompts:
            if self.cfg.use_cache:
                ...
                # do not process them
                if pooled:
                    prefix = hash_prompt(self.cfg.pretrained_model_name_or_path+"@pooled", prompt)
                else:
                    prefix = hash_prompt(self.cfg.pretrained_model_name_or_path, prompt)
    ...
    def load_from_cache(self, prompt, pooled=False):
        if pooled:
            prefix = hash_prompt(self.cfg.pretrained_model_name_or_path+"@pooled", prompt)
        else:
            prefix = hash_prompt(self.cfg.pretrained_model_name_or_path, prompt)
    ...
    def __call__(self) -> PromptProcessorOutput:
        return PromptProcessorOutput(
            ...
            uncond_text_embeddings=self.uncond_text_embeddings,  # negative
