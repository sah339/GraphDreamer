...
class VanillaMLP(nn.Module):
    def __init__(self, dim_in: int, dim_out: int, config: dict):
        ...
        special_init = False  # TODO: clean
        if "special_init" in config: 
            special_init = config["special_init"]
        layers += [
            self.make_linear(self.n_neurons, dim_out, is_first=False, is_last=True, special_init=special_init)
        ]
    ...
    def make_linear(self, dim_in, dim_out, is_first, is_last, special_init=False):
        if is_last and special_init:
            threestudio.info(">>> MLP last layer: special init <<<")
